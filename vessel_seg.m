...
%========segment vessel which is touching more then 1 boundry ===========
%========================and thninning filter=============================
[r, c, ~]=size(img);
T=graythresh(img);
ib = imbinarize(img, T);
ib = imfill(ib,'holes');
...
B=array2table(wh_pts,'VariableNames',{'x','y'});
for i=1:intpt
    A=array2table(s1(pts_position(i)).PixelList,'VariableNames',{'x','y'});
    C2 = intersect(A,B)
    [len_pts , ~]=size(C2);
    if len_pts<6
        npts_position(count)=pts_position(i);
        count=count+1;
    end
end
intpt=length(npts_position);
...
% 
% % T=Threshold_Level(img);
% % T=graythresh(img)
% % ib = imbinarize(img, T+0.08);
% ib = imbinarize(img,'adaptive');
% ib = imfill(ib,'holes');
% 
% s1=regionprops(ib,'BoundingBox','Extrema','PixelList');
% [size_prop,~]=size(s1);
% 
% for i=1:size_prop
%     s1(i).Extrema=int8(s1(i).Extrema);  
